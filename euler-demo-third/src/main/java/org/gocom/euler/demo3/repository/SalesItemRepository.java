...
public interface SalesItemRepository extends JpaRepository<SalesItemEntity, Long>{
	...
	@Query("select s.product as product, s.type as type, sum(s.number) as sumNumber, month(s.createDate) as month "
			+ "from SalesItemEntity s left outer join s.product p where s.product = ?1 and s.type = ?2 group by month(s.createDate) order by s.createDate")
